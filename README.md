...
폼이 제출되면 **김수진 과장(010-8114-4638)** 휴대폰으로 문자가 오도록 설계되어 있습니다.
지금은 **연결 전 상태**(입력 검증만 됨)이며, 아래 파일 순서대로 설정하면 활성화됩니다:

> ### 👉 `api/폼연동-안내.md` 를 따라 하세요 (약 15~20분)

**요약**

1. **알리고(aligo)** 가입 → 발신번호 `010-8114-4638` 사전등록 → API Key 발급 → 소액 충전
2. 구글 시트 새로 만들기 (접수 내용 저장용)
3. **Google Apps Script** 에 `api/apps-script.gs` 붙여넣고 `CONFIG` 값 채우기
4. `_test` 함수 실행 → 문자 오는지 확인
5. **웹 앱으로 배포** → 나온 URL 복사
6. `index.html` 의 `<form id="leadForm" ... data-endpoint="">` 의 `data-endpoint=""` 안에 그 URL 붙여넣기

**동작**

- 제출 → 구글 시트에 한 줄 기록 + 담당자 폰에 문자 + (선택) 이메일 사본
- `fetch()` 비동기 전송이라 페이지 이동 없이 "접수되었습니다" 문구 표시
- `data-endpoint` 가 비어 있으면 검증만 하고 "연동 전" 안내 표시
- 스팸봇 차단 숨은 필드(`_hp`), '방문예약' 선택 시 희망 방문일시 칸 강조 내장

**문자 대신 이메일만** 원하면 알리고 단계를 건너뛰고 `CONFIG.ALERT_EMAIL` 만 채우면 됩니다.

> 개인정보(성명·연락처)를 다루므로 홈페이지는 **HTTPS 도메인**에서 운영하세요.
> `privacy.html` 수탁업체 표에 알리고·Google 을 이미 추가해 두었습니다.
